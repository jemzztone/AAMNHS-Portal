...
class BuildAnalyticsReport
{
    ...
    public function handle(User $user, string $startDate, string $endDate, ?string $sectionId): array
    {
        ...
        $sectionRanking = AttendanceRecord::whereDate('date', '>=', $startDate)->whereDate('date', '<=', $endDate)
            ->when($teacherSectionIds, fn ($q) => $q->whereIn('section_id', $teacherSectionIds))
            ->select('section_id', DB::raw('COUNT(*) as total'), DB::raw("SUM(CASE WHEN status = 'late' THEN 1 ELSE 0 END) as late_count"))
            ->groupBy('section_id')
        ...
        $lateStudents = Student::query()
            ...
            ->when($teacherSectionIds, fn ($q) => $q->whereIn('attendance_records.section_id', $teacherSectionIds))
            ->groupBy('students.id')
            ->orderByDesc('late_count')
            ->take(10)
            ->get();
